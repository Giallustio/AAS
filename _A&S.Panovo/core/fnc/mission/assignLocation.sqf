...
if (time isEqualTo 0) then {
	[_loc] spawn {
		sleep 2;private "_loc";_loc = _this select 0;[(_loc getvariable ["name", "error"]),(_loc getvariable ["id", -1]),position _loc] remoteExec ["btc_fnc_mission_assignLocationNotification",0]; 
	};
} else {[(_loc getvariable ["name", "error"]),(_loc getvariable ["id", -1]),position _loc] remoteExec ["btc_fnc_mission_assignLocationNotification",0];};
...
[_loc, ((_radiusx + _radiusy)/2)] spawn btc_fnc_mission_populateLocation;
...
[_loc,_radiusx,_radiusy] spawn {
	sleep 5;
	[position (_this select 0), [((_this select 1) + btc_loc_radius),((_this select 2) + btc_loc_radius),0,false], [str (btc_enemy_side), "PRESENT", false], ["count thisList < 2", "[] call btc_fnc_mission_locationCaptured", ""]] call btc_fnc_createTrigger;
};
